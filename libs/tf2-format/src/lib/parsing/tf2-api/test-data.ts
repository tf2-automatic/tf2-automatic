export function getBasicItem() {
  return {
    id: 15681438682,
    original_id: 11373475188,
    defindex: 5021,
    level: 5,
    quality: 6,
    inventory: 3221225475,
    quantity: 1,
    origin: 2,
    attributes: [
      {
        defindex: 2046,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 195,
        value: 1065353216,
        float_value: 1,
      },
    ],
  };
}

export function getProfessionalKillstreakItem() {
  return {
    id: 15497962225,
    original_id: 1388332098,
    defindex: 154,
    level: 5,
    quality: 11,
    inventory: 3221225475,
    quantity: 1,
    origin: 8,
    attributes: [
      {
        defindex: 189,
        value: 1093664768,
        float_value: 11,
      },
      {
        defindex: 214,
        value: 0,
        float_value: 0,
      },
      {
        defindex: 2013,
        value: 1157259264,
        float_value: 2003,
      },
      {
        defindex: 2014,
        value: 1086324736,
        float_value: 6,
      },
      {
        defindex: 2025,
        value: 1077936128,
        float_value: 3,
      },
      {
        defindex: 796,
        value: '10 -3 -11',
      },
      {
        defindex: 68,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 67,
        value: 1066192077,
        float_value: 1.10000002384185791,
      },
    ],
  };
}

export function getProfessionalKillstreakKitFabricator() {
  return {
    id: 15443117973,
    original_id: 13666970647,
    defindex: 20003,
    level: 1,
    quality: 6,
    inventory: 2147483703,
    quantity: 1,
    origin: 20,
    attributes: [
      {
        defindex: 2022,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2000,
        is_output: false,
        quantity: 2,
        quality: 6,
        match_all_attribs: true,
        attributes: [
          {
            defindex: 2025,
            value: 1073741824,
            float_value: 2,
          },
        ],
      },
      {
        defindex: 2001,
        is_output: false,
        quantity: 13,
        itemdef: 5706,
        quality: 6,
      },
      {
        defindex: 2002,
        is_output: false,
        quantity: 3,
        itemdef: 5707,
        quality: 6,
      },
      {
        defindex: 2003,
        is_output: false,
        quantity: 4,
        itemdef: 5702,
        quality: 6,
      },
      {
        defindex: 2004,
        is_output: false,
        quantity: 2,
        itemdef: 5704,
        quality: 6,
      },
      {
        defindex: 2005,
        is_output: false,
        quantity: 3,
        itemdef: 5701,
        quality: 6,
      },
      {
        defindex: 2006,
        is_output: true,
        quantity: 1,
        itemdef: 6526,
        quality: 6,
        attributes: [
          {
            defindex: 2014,
            value: 1088421888,
            float_value: 7,
          },
          {
            defindex: 2013,
            value: 1157267456,
            float_value: 2004,
          },
          {
            defindex: 2012,
            value: 1137377280,
            float_value: 406,
          },
        ],
      },
    ],
  };
}

export function getStrangifierChemistrySet() {
  return {
    id: 15510439400,
    original_id: 2124822184,
    defindex: 20005,
    level: 5,
    quality: 6,
    inventory: 3221225475,
    quantity: 1,
    origin: 0,
    attributes: [
      {
        defindex: 2000,
        is_output: false,
        quantity: 4,
        itemdef: 331,
        quality: 6,
      },
      {
        defindex: 2001,
        is_output: false,
        quantity: 1,
        itemdef: 61,
        quality: 6,
      },
      {
        defindex: 2002,
        is_output: false,
        quantity: 1,
        itemdef: 649,
        quality: 6,
      },
      {
        defindex: 2003,
        is_output: false,
        quantity: 1,
        itemdef: 42,
        quality: 6,
      },
      {
        defindex: 2004,
        is_output: false,
        quantity: 1,
        itemdef: 36,
        quality: 6,
      },
      {
        defindex: 2005,
        is_output: false,
        quantity: 1,
        itemdef: 38,
        quality: 6,
      },
      {
        defindex: 2006,
        is_output: false,
        quantity: 1,
        itemdef: 43,
        quality: 11,
      },
      {
        defindex: 2007,
        is_output: true,
        quantity: 1,
        itemdef: 6522,
        quality: 6,
        attributes: [
          {
            defindex: 2012,
            value: 1135312896,
            float_value: 343,
          },
        ],
      },
      {
        defindex: 2031,
        value: 1073741824,
        float_value: 2,
      },
    ],
  };
}

export function getDuelingMinigame() {
  return {
    id: 15568852549,
    original_id: 15487882462,
    defindex: 241,
    level: 5,
    quality: 6,
    inventory: 3221225475,
    quantity: 5,
    origin: 0,
    attributes: [
      {
        defindex: 195,
        value: 1065353216,
        float_value: 1,
      },
    ],
  };
}

export function getSpelledItem() {
  return {
    id: 7453028434,
    original_id: 369376082,
    defindex: 44,
    level: 15,
    quality: 11,
    inventory: 2147485760,
    quantity: 1,
    origin: 3,
    style: 0,
    attributes: [
      {
        defindex: 189,
        value: 1093664768,
        float_value: 11,
      },
      {
        defindex: 214,
        value: 10,
        float_value: 1.40129846432481707e-44,
      },
      {
        defindex: 815,
        value: 1,
        float_value: 1.40129846432481707e-45,
      },
      {
        defindex: 1009,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2025,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2053,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 796,
        value: '10 0 -6',
      },
      {
        defindex: 38,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 125,
        value: 3245342720,
        float_value: -15,
      },
      {
        defindex: 551,
        value: 1065353216,
        float_value: 1,
      },
    ],
  };
}

export function getProfessionalKillstreakItemWithSpellAndParts() {
  return {
    id: 15795067590,
    original_id: 1563805932,
    defindex: 191,
    level: 1,
    quality: 11,
    inventory: 3221225475,
    quantity: 1,
    origin: 8,
    custom_name: 'Henny',
    attributes: [
      {
        defindex: 214,
        value: 0,
        float_value: 0,
      },
      {
        defindex: 379,
        value: 0,
        float_value: 0,
      },
      {
        defindex: 380,
        value: 1107558400,
        float_value: 33,
      },
      {
        defindex: 381,
        value: 0,
        float_value: 0,
      },
      {
        defindex: 382,
        value: 1097859072,
        float_value: 15,
      },
      {
        defindex: 1009,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2013,
        value: 1157275648,
        float_value: 2005,
      },
      {
        defindex: 2014,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2025,
        value: 1077936128,
        float_value: 3,
      },
      {
        defindex: 500,
        value: 'Henny',
      },
      {
        defindex: 796,
        value: '10 0 -14',
      },
    ],
  };
}

export function getElevatedUnusualSkin() {
  return {
    id: 15544141666,
    original_id: 4148729994,
    defindex: 15052,
    level: 1,
    quality: 15,
    inventory: 2147483826,
    quantity: 1,
    origin: 8,
    equipped: [
      {
        class: 3,
        slot: 0,
      },
    ],
    attributes: [
      {
        defindex: 185,
        value: 1581038230,
        float_value: 3.3998367378857001e18,
      },
      {
        defindex: 214,
        value: 14,
        float_value: 1.9618178500547439e-44,
      },
      {
        defindex: 379,
        value: 5058,
        float_value: 7.08776763255492474e-42,
      },
      {
        defindex: 380,
        value: 1118044160,
        float_value: 82,
      },
      {
        defindex: 381,
        value: 9,
        float_value: 1.26116861789233536e-44,
      },
      {
        defindex: 382,
        value: 1102577664,
        float_value: 23,
      },
      {
        defindex: 383,
        value: 0,
        float_value: 0,
      },
      {
        defindex: 384,
        value: 1107820544,
        float_value: 34,
      },
      {
        defindex: 725,
        value: 1045220557,
        float_value: 0.200000002980232239,
      },
      {
        defindex: 1007,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 1009,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2013,
        value: 1157251072,
        float_value: 2002,
      },
      {
        defindex: 2014,
        value: 1082130432,
        float_value: 4,
      },
      {
        defindex: 2025,
        value: 1077936128,
        float_value: 3,
      },
      {
        defindex: 724,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 796,
        value: '10 -3 -10',
      },
      {
        defindex: 719,
        value: 'models/weapons/c_models/stattrack.mdl',
      },
      {
        defindex: 731,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2028,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 749,
        value: 1058642330,
        float_value: 0.60000002384185791,
      },
      {
        defindex: 745,
        value: 1,
        float_value: 1.40129846432481707e-45,
      },
      {
        defindex: 834,
        value: 52,
        float_value: 7.28675201448904877e-44,
      },
    ],
  };
}

export function getProfessionalKillstreakKit() {
  return {
    id: 15080836060,
    original_id: 7044205050,
    defindex: 6526,
    level: 5,
    quality: 6,
    inventory: 2147484538,
    quantity: 1,
    origin: 22,
    flag_cannot_craft: true,
    attributes: [
      {
        defindex: 2012,
        value: 1124073472,
        float_value: 128,
      },
      {
        defindex: 2013,
        value: 1157251072,
        float_value: 2002,
      },
      {
        defindex: 2014,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 806,
        value: '0.285 0.43 0.35',
      },
      {
        defindex: 449,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 743,
        value: 1065353216,
        float_value: 1,
      },
      {
        defindex: 2025,
        value: 1077936128,
        float_value: 3,
      },
    ],
  };
}

export function getItemWithBuggedWear() {
  return {
    id: 6422121129,
    original_id: 6270791978,
    defindex: 201,
    level: 99,
    quality: 15,
    inventory: 2147485100,
    quantity: 1,
    origin: 28,
    style: 0,
    attributes: [
      {
        defindex: 211,
        value: 1513324800,
        float_value: 12631464457797632,
      },
      { defindex: 725, value: 0, float_value: 0 },
      { defindex: 834, value: 390, float_value: 5.4650640108667866e-43 },
      {
        defindex: 866,
        value: 1975813521,
        float_value: 4.982418129070686e32,
      },
      { defindex: 867, value: 1, float_value: 1.401298464324817e-45 },
      { defindex: 719, value: 'models/weapons/c_models/stattrack.mdl' },
      { defindex: 731, value: 1065353216, float_value: 1 },
      {
        defindex: 724,
        value: 1059917398,
        float_value: 0.6759999990463257,
      },
      { defindex: 796, value: '10 0 -10' },
    ],
  };
}
